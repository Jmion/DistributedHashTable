...
//------------------- End Debug block--------------------------
	...
	while (index < ring->size && node_cmp_sha(&ring->nodes[index], &artificial_node) < 0) {
		++index;
	}
	//if key sha is greater than all the nodes sha, index is too large
	index = index % ring->size;
	size_t nbNodes = 0;
	size_t i = 0;
	debug_print("%s", "Key added to following nodes : ");
	while (nbNodes < wanted_list_size && i < ring->size) {
	//correcteur: if i is > ring->size and not all nodes have been seen and we still want nodes you need to loop again
		size_t flag = 0;
